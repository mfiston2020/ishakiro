        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <div class="card">
                        <div class="body">
                            <form action="{{ route('agent.document.search')}}" method="get">
                            ...
                            <tbody>@foreach ($results as $item)
                                <tr>
                                    <td class="max_ellipsis">
                                        <h5 class=""><a href="{{ route('agent.myfound.document',Crypt::encryptString($item->id))}}" style="color: #1272BA"><strong>{{$query}}</strong> Search result!</a></h5>
                                        <a class="link" href="{{ route('agent.myfound.document',Crypt::encryptString($item->id))}}" style="color:#5CC2D6">
                                            {{\App\DocumentType::where(['id'=>$item->document_type])->pluck('type')->first()}} : {{substr_replace($item->document_number,'************',2)}}{{substr($item->document_number,-2)}}
